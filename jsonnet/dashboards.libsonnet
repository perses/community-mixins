...
function (params) {
    ...
    local alertmanagerOverview = if std.member(cd.config.components, 'alertmanager') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/alertmanager/alertmanager-overview.json',
        cd.config.namespace,
    ...
    local blackboxExporter = if std.member(cd.config.components, 'blackbox-exporter') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/blackbox-exporter/blackbox-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local etcdOverview = if std.member(cd.config.components, 'etcd') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/etcd/etcd-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local apiServerOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/api-server-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local kubeControllerManagerOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/controller-manager-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local kubeSchedulerOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/scheduler-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local kubeProxyOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/proxy-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local kubeletOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubelet-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local clusterNetworkingOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-cluster-networking-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local clusterResourceOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-cluster-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local multiClusterOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-multi-cluster-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local namespaceNetworkingOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-namespace-networking-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local namespaceResourceOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-namespace-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local nodeOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-node-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local persistentVolumeOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-persistent-volume-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local podNetworkingOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-pod-networking-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local podOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-pod-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local workloadOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-workload-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local workloadNetworkingOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-workload-networking-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local workloadNSResourceOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-workload-ns-resources-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local workloadNSNetworkingOverview = if std.member(cd.config.components, 'kubernetes') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/kubernetes/kubernetes-workload-ns-networking-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local nodeExporterClusterUSE = if std.member(cd.config.components, 'node-exporter') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/node-exporter/node-exporter-cluster-use-method.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local nodeExporterNodes = if std.member(cd.config.components, 'node-exporter') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/node-exporter/node-exporter-nodes.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local persesOverview = if std.member(cd.config.components, 'perses') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/perses/perses-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local prometheusOverview = if std.member(cd.config.components, 'prometheus') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/prometheus/prometheus-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local prometheusRemoteWrite = if std.member(cd.config.components, 'prometheus') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/prometheus/prometheus-remote-write.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local thanosCompactOverview = if std.member(cd.config.components, 'thanos') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/thanos/thanos-compact-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local thanosRulerOverview = if std.member(cd.config.components, 'thanos') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/thanos/thanos-ruler-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local thanosStoreOverview = if std.member(cd.config.components, 'thanos') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/thanos/thanos-store-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local thanosQueryOverview = if std.member(cd.config.components, 'thanos') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/thanos/thanos-query-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local thanosReceiveOverview = if std.member(cd.config.components, 'thanos') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/thanos/thanos-receive-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
    ...
    local thanosQueryFrontendOverview = if std.member(cd.config.components, 'thanos') then overrides.overrideDashboard(
        import '../examples/dashboards/json/operator/thanos/thanos-query-frontend-overview.json',
        cd.config.namespace,
        cd.config.commonLabels,
        cd.config.datasource
    ) else {},
